...
def generate_response(model, tokenizer, messages, device, max_new_tokens=1000):
...
def title_formator(retrieved_titles, retrieved_bodies, title):
    messages = [
        ...
        {"role": "assistant", "content": retrieved_bodies[0]},
        {"role": "user", "content": f"# {retrieved_titles[1]}"},
        {"role": "assistant", "content": retrieved_bodies[1]},
        {"role": "user", "content": f"# {title}"}
    ]
    return messages

input_file = "Retrieval/qwen3-embedding-8B/TBtest.jsonl"
output_file = "RAFG/Results/gemma-3-4b-it_TBtest.jsonl"
